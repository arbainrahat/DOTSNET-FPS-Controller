...
namespace DOTSNET
{
    ...
    public class NetworkServerStatisticsSystem : SystemBase
    {
        ...
        void OnReceive(int _, ArraySegment<byte> segment)
        {
            ++intervalReceivedPackets;
            intervalReceivedBytes += segment.Count;
        }

        void OnSend(int _, ArraySegment<byte> segment)
        {
            ++intervalSentPackets;
            intervalSentBytes += segment.Count;
        }
